    ...
    def compute_ppo_loss(self, model_output, data):
        ...
        if self.config.use_kl_loss:
            ...
            if "difficulty_source" in data:
                ...
                kld_original = kl_penalty(logprob=log_prob, ref_logprob=ref_log_prob, kl_penalty=self.config.kl_loss_type)
                kl_loss_original = agg_loss(loss_mat=kld_original, loss_mask=response_mask, loss_agg_mode=self.config.loss_agg_mode)
                ...
                if np.any(hard_mask):
                    ...
                    if hard_response_mask.sum() > 0:
                        ...
                        hard_kl_loss_original = agg_loss(loss_mat=hard_kld_original, loss_mask=hard_response_mask, loss_agg_mode=self.config.loss_agg_mode)
                        ...
                        hard_kl_loss_scaled = agg_loss(loss_mat=hard_kld_scaled, loss_mask=hard_response_mask, loss_agg_mode=self.config.loss_agg_mode)
                        ...
                        if "kl_in_loss_scale" in data:
                            ...
                            metrics["actor/kl_coeff_hard_scaled"] = self.config.kl_loss_coef
                ...
                if np.any(non_hard_mask):
                    ...
                    if non_hard_response_mask.sum() > 0:
                        ...
                        non_hard_kl_loss_original = agg_loss(loss_mat=non_hard_kld_original, loss_mask=non_hard_response_mask, loss_agg_mode=self.config.loss_agg_mode)
                        ...
                        non_hard_kl_loss_scaled = agg_loss(loss_mat=non_hard_kld_scaled, loss_mask=non_hard_response_mask, loss_agg_mode=self.config.loss_agg_mode)
                        ...
                        if "kl_in_loss_scale" in data:
                            ...
                            metrics["actor/kl_coeff_non_hard_scaled"] = self.config.kl_loss_coef
