...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Add custom reward metrics from reward_extra_info
    for key, values in batch.non_tensor_batch.items():
        # Process reward metrics: all fields ending with _reward, plus core reward fields
        is_reward_metric = (
            key.endswith('_reward') or      # 所有以 _reward 结尾的字段（用户自定义奖励）
            key in ['acc', 'score']         # 核心奖励字段（系统预期的）
        )
        
        if is_reward_metric and isinstance(values, np.ndarray) and values.dtype.kind in 'fi':
            try:
                # 确保tensor在正确的设备上
                tensor_values = torch.from_numpy(values).float().to(aborted_mask.device)
                non_aborted_indices = torch.nonzero(~aborted_mask, as_tuple=True)[0]
                
                if len(non_aborted_indices) > 0:
                    non_aborted_values = tensor_values[non_aborted_indices]
                    metrics[f"train/{key}/mean"] = torch.mean(non_aborted_values).detach().item()
                    metrics[f"train/{key}/max"] = torch.max(non_aborted_values).detach().item()
                    metrics[f"train/{key}/min"] = torch.min(non_aborted_values).detach().item()
            except Exception as e:
                # Skip if there's any error processing this metric
                print(f"Warning: Could not process reward metric {key}: {e}")
                continue
