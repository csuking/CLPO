...
from verl.trainer.evaluation.sft_evaluator import SFTEvaluator
...
class FSDPSFTTrainer:
    def __init__(
        ...
        # 初始化评测器
        self.evaluator = None
        if hasattr(self.config.trainer, "evaluation") and self.config.trainer.evaluation.enable:
            # 添加评测数据路径到配置中
            eval_config = self.config.trainer.evaluation.copy()
            if hasattr(self.config.data, "val_files"):
                eval_config["eval_data_path"] = self.config.data.val_files
            ...
            if self.device_mesh.get_rank() == 0:
                print(f"Initialized SFT evaluator with generation config: {self.evaluator.generation_config}")
    ...
    def generation_evaluation_step(self, model, val_dataloader, global_step: int) -> Dict[str, float]:
        """基于生成的评测"""
        if self.evaluator is None:
            return {}
        
        try:
            metrics = self.evaluator.evaluate(model, val_dataloader, global_step)
            if self.device_mesh.get_rank() == 0:
                print(f"Generation evaluation metrics: {metrics}")
            return metrics
        ...
        for epoch in range(start_epoch, self.config.trainer.total_epochs):
            ...
            for step_in_epoch, data in enumerate(
                ...
                if is_last_step or (self.config.trainer.test_freq > 0 and is_valid_step):
                    if rank == 0:
                        print(f"\n--- Starting evaluation at step {global_step} ---")
                    ...
                    if self.evaluator is not None:
                        eval_metrics = self.generation_evaluation_step(self.fsdp_model, self.val_dataloader, global_step)
                        if eval_metrics and rank == 0:
                            tracking.log(data=eval_metrics, step=global_step)
                            if last_valid_metric is None:
                                last_valid_metric = {}
                            last_valid_metric.update(eval_metrics)
